...
func NewProposerSnapshotFromState(proposerStore *ProposerSnapshotStore,
	backend polytypes.Polybft, dbTx *bolt.Tx) (*ProposerSnapshot, error) {
	snapshot, err := proposerStore.getProposerSnapshot(dbTx)
	if err != nil {
		return nil, err
	}
	...
	if snapshot == nil {
		...
		genesisValidatorsSet, err := backend.GetValidatorsWithTx(0, nil, dbTx)
		if err != nil {
			return nil, err
		}
...
func NewProposerCalculator(cfg *config.Runtime, logger hclog.Logger,
	...
	snap, err := NewProposerSnapshotFromState(proposerStore, backend, dbTx)
	if err != nil {
		return nil, err
	}
	...
	blockNumber := blockchain.CurrentHeader().Number
...
func (pc *ProposerCalculator) update(blockNumber uint64, dbTx *bolt.Tx) error {
	...
	if err := pc.state.writeProposerSnapshot(pc.snapshot, dbTx); err != nil {
...
func (pc *ProposerCalculator) updatePerBlock(blockNumber uint64, dbTx *bolt.Tx) error {
	...
	_, extra, err := helpers.GetBlockData(blockNumber, pc.blockchain)
	if err != nil {
	...
	if extra.Validators != nil && !extra.Validators.IsEmpty() {
		newValidatorSet, err = pc.backend.GetValidatorsWithTx(blockNumber, nil, dbTx)
		if err != nil {
